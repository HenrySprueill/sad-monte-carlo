...
def partial_volume(x1, N, prec = 1000, R=1):
    ...
    return np.sum(integrand(stencil)) * (stencil[1] - stencil[0])
...
def D(e):
    ...
    return V(n)*(R_small**n/2*np.sqrt(e/h_small+1)**(n-2)/h_small + R_big**n/2*np.sqrt(e/h_big+1)**(n-2)/h_big)/total_volume
D = np.vectorize(D)
